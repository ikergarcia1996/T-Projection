...
accelerate launch --num_processes 2 --mixed_precision  bf16 seq2seq_t5.py \
  ...
  --dev_tsv datasets/test_data/en.europarl.conll \
  --test_source_tsv datasets/test_data/en.europarl.conll \
  --test_target_tsv datasets/test_data/es.europarl.conll \
  --num_beams 10 \
  ...
  --output_dir datasets/test_data/mt5-large \
  --seed 42 \
  --eval_every 50 \
...
python3 calculate_scores_nmts.py \
  ...
  --output_path datasets/test_data/mt5-large/es.europarl.json \
  --source_lang en \
  --target_lang es \
  --normalize \
  --both_directions
