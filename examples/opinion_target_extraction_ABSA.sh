...
--train_tsv datasets/data/en.ote.train.test.tsv \
--dev_tsv datasets/data/en.ote.train.tsv \
--test_source_tsv \
  datasets/data/en.ote.train.tsv \
  datasets/data/en.ote.train.tsv \
  datasets/data/en.ote.train.tsv \
  datasets/data/en.ote.train.tsv \
--test_target_tsv \
  datasets/data/es.ote.train.tsv \
  datasets/data/fr.ote.train.tsv \
  datasets/data/ru.ote.train.tsv \
  datasets/data/tr.ote.train.tsv \
--num_beams 15 \
...
do
  ...
  --jsonl_path results/ote/"$lang".ote.train.jsonl \
  --model_name_or_path facebook/m2m100-12B-last-ckpt \
  --output_path results/ote/m2m100-12B.en2"$lang".train.json \
  --source_lang en \
  --target_lang "$lang" \
  --normalize \
  --both_directions
  ...
  --jsonl_path results/ote/"$lang".ote.train.jsonl \
  --dictionary_path results/ote/m2m100-12B.en2"$lang".train.json \
  --output_path results/ote/en2"$lang".train.tsv \
  --gold_tsv datasets/data/"$lang".ote.train.tsv
